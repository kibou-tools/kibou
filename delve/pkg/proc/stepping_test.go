	...
	withTestProcessArgs("rangeoverfunc", t, ".", []string{}, 0, func(p *proc.Target, grp *proc.TargetGroup, fixture protest.Fixture) {
		t.Run("TestTrickyIterAll1", func(t *testing.T) {
			testseq2intl(t, fixture, grp, p, nil, []seqTest{
				...
				h.ifcond(runtime.GOARCH == "arm64" && goversion.ProducerAfterOrEqual(p.BinInfo().Producer(), 1, 25), h.nop, nx(25)),
				nx(26),
				nx(27), // for _, x := range ...
		...
		t.Run("TestTrickyIterAll2", func(t *testing.T) {
			testseq2intl(t, fixture, grp, p, nil, []seqTest{
				...
				h.ifcond(runtime.GOARCH == "arm64" && goversion.ProducerAfterOrEqual(p.BinInfo().Producer(), 1, 25), h.nop, nx(38)),
				nx(39),
				nx(40), // for _, x := range...
				nx(41),
				nx(42),
	...
	withTestProcessArgs("rangeoverfunc", t, ".", []string{}, protest.EnableInlining, func(p *proc.Target, grp *proc.TargetGroup, fixture protest.Fixture) {
		...
		t.Run("TestTrickyIterAll1", func(t *testing.T) {
			testseq2intl(t, fixture, grp, p, nil, []seqTest{
				h.funcBreak(t, "main.TestTrickyIterAll"),
				{contContinue, 24}, // TestTrickyIterAll
				h.ifcond(runtime.GOARCH != "arm64" || !goversion.ProducerAfterOrEqual(p.BinInfo().Producer(), 1, 27), nx(25), h.nop),
				nx(26),
				nx(27), // for _, x := range ...
				assertLocals(t, "trickItAll", "i"),
				assertEval(t, "i", "0"),
		...
		t.Run("TestTrickyIterAll2", func(t *testing.T) {
			testseq2intl(t, fixture, grp, p, nil, []seqTest{
				h.funcBreak(t, "main.TestTrickyIterAll2"),
				{contContinue, 37}, // TestTrickyIterAll2
				h.ifcond(runtime.GOARCH != "arm64" || !goversion.ProducerAfterOrEqual(p.BinInfo().Producer(), 1, 27), nx(38), h.nop),
				nx(39),
				nx(40), // for _, x := range...
				nx(41),
				nx(42),
		...
		t.Run("TestGotoA1", func(t *testing.T) {
			testseq2intl(t, fixture, grp, p, nil, []seqTest{
				h.funcBreak(t, "main.TestGotoA1"),
				{contContinue, 192},
				h.ifcond(runtime.GOARCH != "arm64" || !goversion.ProducerAfterOrEqual(p.BinInfo().Producer(), 1, 27), nx(193), h.nop),
				nx(194), // for _, x := range (x == -1)
				nx(195), // result = append(result, x)
				nx(196), // if x == -4
				nx(199), // for _, y := range (y == 1)
		...
		t.Run("TestGotoB1", func(t *testing.T) {
			testseq2intl(t, fixture, grp, p, nil, []seqTest{
				h.funcBreak(t, "main.TestGotoB1"),
				{contContinue, 211},
				h.ifcond(runtime.GOARCH != "arm64" || !goversion.ProducerAfterOrEqual(p.BinInfo().Producer(), 1, 27), nx(212), h.nop),
				nx(213), // for _, x := range (x == -1)
				nx(214), // result = append(result, x)
				nx(215), // if x == -4
				nx(218), // for _, y := range (y == 1)
