...
func BuildFixture(t testing.TB, name string, flags BuildFlags) Fixture {
	...
	// NOTE(kibou): Delve's tests themselves run with GOROOT set
	// for kibou (i.e. go test uses kibou's binary). However,
	// we want to build the test fixtures using different toolchains,
	// to make sure that our fork of Delve can continue to use the
	// debuginfo generated by upstream Go toolchain, as kibou should
	// support a superset of things supported by golang/go.
	goTool := os.Getenv("DELVE_TEST_GO")
	if goTool == "" {
		goTool = "go"
	}
	cmd := exec.Command(goTool, buildFlags...)
	cmd.Dir = dir
	env := os.Environ()
	if goTool != "go" {
		goRoot := filepath.Dir(filepath.Dir(goTool))
		if goRoot == "." {
			t.Fatalf("Failed to determine GOROOT for building fixture from DELVE_TEST_GO=%v", goTool)
		}
		env = overrideEnv(env, "GOROOT", goRoot)
	}
	cmd.Env = env
